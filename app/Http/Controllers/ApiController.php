...
getItemLight(Request $request)
{
   ...
   public function 
getItemDetailsLight(Request $request, $id)
{
    try {
...
public function ramagetItem(Request $request)
{
    $validator = Validator::make($request->all(), [
        'limit' => 'nullable|integer',
        'offset' => 'nullable|integer',
...
{
    \Log::info("sendMessage called with data: " . json_encode($request->all(), JSON_PRETTY_PRINT));

    // تسجيل معلومات مفصلة عن حقل file للتصحيح
    \Log::info("file hasFile: " . ($request->hasFile('file') ? 'YES real file' : 'NO'));
    \Log::info("file filled: " . ($request->filled('file') ? 'YES' : 'NO'));
    if ($request->filled('file')) {
        $raw = $request->input('file');
        \Log::info("file type: " . gettype($raw));
        \Log::info("file length: " . (is_string($raw) ? strlen($raw) : 'not string'));
        \Log::info("file starts with: " . (is_string($raw) ? substr($raw, 0, 100) : json_encode($raw)));
    }
    ...
    try {
        ...
        // معالجة الصورة (كما هو بدون تغيير)
        if ($request->hasFile('file') && $request->file('file')->isValid()) {
            $filePath = ChatFileService::compressAndUpload($request->file('file'), 'chat');
            \Log::info("Real file compressed & uploaded: " . $filePath);
        } elseif ($request->filled('file') && is_string($request->input('file'))) {
            $rawFile = $request->input('file');
            // إذا كان base64
            if (str_starts_with($rawFile, 'data:image')) {
                $filePath = $this->saveBase64Image($rawFile);
                if ($filePath) {
                    \Log::info("Base64 image saved: " . $filePath);
                } else {
                    \Log::warning("Failed to save base64 image");
                    $filePath = '';
                }
            } else {
                // أي string آخر (path محلي، json...) → نحفظه كما هو
                $filePath = $rawFile;
                \Log::info("String file saved as-is (may not display): " . substr($rawFile, 0, 100));
            }
        }

        // معالجة الصوت (كما هو)
        if ($request->hasFile('audio') && $request->file('audio')->isValid()) {
            $audioPath = ChatFileService::compressAndUpload($request->file('audio'), 'chat');
        } elseif ($request->filled('audio') && is_string($request->input('audio'))) {
            $audioPath = $request->input('audio');
        }
        ...
        // ────────────── قسم الإشعارات (معدل ليكون بنفس الآلية القديمة بالضبط) ──────────────
        $receiver_id = $itemOffer->seller_id == $user->id ? $itemOffer->buyer_id : $itemOffer->seller_id;
        $userType    = $itemOffer->seller_id == $user->id ? "Seller" : "Buyer";
        ...
        $fcmMsg = [
            ...
            'item_offer_id'     => $itemOffer->id,
            'item_offer_amount' => $itemOffer->amount ?? 0,
            'unread_count'      => $unreadMessagesCount,
            'type'              => $notificationPayload['message_type'] ?? null,
            'message_type_temp' => $notificationPayload['message_type'] ?? null,
        ];

        // message_type is reserved keyword in FCM so removed here
        unset($fcmMsg['message_type']);
        unset($fcmMsg['message_type_temp']);

        $receiverFCMTokens = UserFcmToken::where('user_id', $receiver_id)
            ->pluck('fcm_token')
            ->toArray();

        // بناء نص الإشعار بنفس الطريقة القديمة بالضبط
        $notificationBody = $request->message;

        if (!$notificationBody) {
            if ($request->hasFile('audio') || ($request->filled('audio') && is_string($request->input('audio')))) {
                $notificationBody = [
                    'en' => '🎵 New voice message',
                    'ar' => '🎵 رسالة صوتية جديدة'
                ];
            } elseif ($request->hasFile('file') || ($request->filled('file') && is_string($request->input('file')))) {
                $notificationBody = [
                    'en' => '📎 New file',
                    'ar' => '📎 ملف جديد'
                ];
            ...
            $hasFile  = $request->hasFile('file') || ($request->filled('file') && is_string($request->input('file')));
            $hasAudio = $request->hasFile('audio') || ($request->filled('audio') && is_string($request->input('audio')));
        ...
        return ResponseService::successResponse(
            ...
            $chat,
            ['debug' => $notification ?? 'No notification sent']
        );
...
/**
 * دالة لحفظ صورة base64 إذا وجدت
 */
private function saveBase64Image(string $base64): string
{
    ...
    try {
        $extension = 'jpg';
        if (str_contains($base64, 'png')) $extension = 'png';
        if (str_contains($base64, 'jpeg') || str_contains($base64, 'jpg')) $extension = 'jpg';
        $data = substr($base64, strpos($base64, ',') + 1);
        $imageData = base64_decode($data);
        if ($imageData === false) return '';
        $fileName = 'chat_img_' . time() . '_' . uniqid() . '.' . $extension;
        $path = 'chat/' . $fileName;
        Storage::disk('public')->put($path, $imageData);
        return $path;
    } catch (\Exception $e) {
        \Log::warning("Base64 save failed: " . $e->getMessage());
        return '';
    }
}
...
/*
private function buildNotificationBody(Request $request): array
{
    $message = $request->input('message', '');

    $hasFile  = $request->hasFile('file') || $request->filled('file');
    $hasAudio = $request->hasFile('audio') || $request->filled('audio');

    if (empty($message)) {
        if ($hasAudio) return ['en' => '🎵 New voice message', 'ar' => '🎵 رسالة صوتية جديدة'];
        if ($hasFile)  return ['en' => '📎 New file', 'ar' => '📎 ملف جديد'];
        return ['en' => 'New message', 'ar' => 'رسالة جديدة'];
    }
    ...
    return [
        'en' => trim($prefixEn . $message),
        'ar' => trim($prefixAr . $message),
    ];
}
*/
...
/*
 public function oldsendMessage(Request $request) {
...
//         \Log::info("User found: " . $user->id . " - " . $user->name);
        ...
        //    $user = User::find($request->sender_id);
            ...
            } else {item_offer_id'     => $itemOffer->id,
                'item_offer_amount' => $itemOffer->amount,
                'type'              => $notificationPayload['message_type'],
                'message_type_temp' => $notificationPayload['message_type'],
                'unread_count'      => $unreadMessagesCount
...
{
    // 1️⃣ حدد اللغة من الهيدر أو استخدم AR افتراضياً
    $lang = strtoupper($request->header('Accept-Language') ?? 'AR');
    if ($lang !== 'AR') {
        $lang = 'en'; // كل شيء غير AR يعتبر إنجليزي
    }

    // 2️⃣ مصفوفة الرسائل داخل الكنترولر
    $messages = [
        'AR' => [
            'already_reviewed' => 'لقد قمت بتقييم هذا الإعلان مسبقاً',
            'review_submitted' => 'تم إرسال تقييمك بنجاح',
            'not_buyer'        => 'يمكنك تقييم الإعلانات التي قمت بشرائها فقط',
            'not_sold_out'     => "يجب أن يكون الإعلان مباعاً قبل أن تتمكن من تقييمه",
        ],
        'en' => [
            'already_reviewed' => 'You have already reviewed this item.',
            'review_submitted' => 'Your review has been submitted successfully.',
            'not_buyer'        => 'You can only review items that you have purchased.',
            'not_sold_out'     => "The item must be marked as 'sold out' before you can review it.",
        ],
    ];
    ...
    try {
        ...
        if ($item->sold_to !== auth()->id()) {
            return ResponseService::errorResponse($messages[$lang]['not_buyer']);
        }

        if ($item->status !== 'sold out') {
            return ResponseService::errorResponse($messages[$lang]['not_sold_out']);
        }
        ...
        if ($existingReview) {
            return ResponseService::errorResponse($messages[$lang]['already_reviewed']);
        }
        ...
        return ResponseService::successResponse($messages[$lang]['review_submitted'], $review);
    ...
    public function oldaddItemReview(Request $request) {
        $validator = Validator::make($request->all(), [
