...
class CustomersController extends Controller {
    ...
    public function show(Request $request) {
        ...
        if ($request->notification_list) {
            $sql = User::role('User')
                       ->where('is_verified', 1) // عرض فقط المستخدمين الذين تم التحقق منهم
                       ->orderBy($sort, $order)
                       ->has('fcm_tokens')
        ...
        foreach ($result as $row) {
            ...
            if (config('app.demo_mode')) {
                if (!empty($row->mobile)) {
                    $tempRow['mobile'] = substr($row->mobile, 0, 3)
                                         . str_repeat('*', (strlen($row->mobile) - 5))
                                         . substr($row->mobile, -2);
                }
                if (!empty($row->email)) {
                    $tempRow['email'] = substr($row->email, 0, 3)
                                         . '****' 
                                         . substr($row->email, strpos($row->email, "@"));
                }
            }
