...
Route::middleware(['auth','can:csv-upload'])->group(function () {
    // صفحة رفع CSV + صور
    Route::get('/admin/csv-upload', [CsvUploadController::class, 'index'])->name('admin.csv.upload');

    // رفع CSV + صور
    Route::post('/admin/csv-upload', [CsvUploadController::class, 'upload'])->name('admin.csv.upload.post');

    // إنشاء مستخدم جديد إذا الإيميل غير موجود
    Route::post('/admin/create-user', [CsvUploadController::class, 'createUser'])->name('admin.create.user');
});
// صفحة عرض البانرز + إنشاء جديد
...
// جلب البيانات للجدول (Bootstrap Table AJAX)
...
Route::group(['middleware'=>['auth']], function(){
    Route::get('/whatsapp', function(){ return view('whatsapp.send'); })->name('whatsapp.send');
    Route::post('/whatsapp/reset-session', [App\Http\Controllers\WhatsappController::class, 'resetSession'])->name('whatsapp.reset');
    Route::get('/whatsapp/qr', [App\Http\Controllers\WhatsappController::class, 'getQR'])->name('whatsapp.qr');
});
