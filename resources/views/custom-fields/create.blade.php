                ...
                <div class="col-md-6 col-sm-12">
                    <div class="card">
                        ...
                        <div class="card-body mt-2">
                            ...
                            {{-- Translation Section for Custom Fields --}}
                                    ...
                                    @foreach($languages as $key => $language)
                                        <div class="col-md-6 form-group">
                                            <label for="name_{{$language->id}}" class="form-label">
                                                {{ ($key + 1) . ". " . $language->name }} ({{ __("Field Name") }}):
                                            </label>
                                            <input type="text" name="translations[{{$language->id}}][name]" id="name_{{$language->id}}" class="form-control" value="">
                                        </div>

                                        <div class="col-md-6 form-group" id="values_translation_{{$language->id}}" style="display:none;">
                                            <label for="values_{{$language->id}}" class="form-label">
                                                {{ ($key + 1) . ". " . $language->name }} ({{ __("Field Values") }}):
                                            </label>
                                            <select id="values_{{$language->id}}" name="translations[{{$language->id}}][values][]" 
                                                    class="select2 w-100 full-width-select2" multiple="multiple" 
                                                    data-tags="true" data-token-separators="[',']" data-placeholder="{{__("Select an option")}}" 
...
@section('js')
    <script>
      ...
      // Translation values show/hide
      const fieldTypeSelect = document.getElementById('type');
      const languages = @json($languages);

      fieldTypeSelect.addEventListener('change', function() {
          const showValues = ['checkbox', 'radio', 'dropdown'].includes(this.value);
          languages.forEach(lang => {
              const div = document.getElementById('values_translation_' + lang.id);
              if(div) div.style.display = showValues ? 'block' : 'none';
          });
      });
    </script>
@endsection
